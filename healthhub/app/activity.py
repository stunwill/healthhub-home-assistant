...
def list_exercise(profile_id: str, db: DbSession, start: date | None = None, end: date | None = None) -> list[ExerciseEntry]:
...
def create_exercise(profile_id: str, payload: ExerciseCreate, db: DbSession) -> ExerciseEntry:
    ...
    entry = ExerciseEntry(profile_id=profile_id, activity_name=payload.activity_name.strip(), duration_minutes=payload.duration_minutes, calories_burned=payload.calories_burned, completed_at=payload.completed_at.astimezone(timezone.utc), source="manual", notes=payload.notes)
    db.add(entry)
    db.commit()
    db.refresh(entry)
    return entry
...
def list_weights(profile_id: str, db: DbSession, days: int = Query(default=90, ge=1, le=730)) -> list[WeightEntry]:
    ...
    first, last = range_bounds(start_date, end_date, profile.timezone)
    statement = select(WeightEntry).where(WeightEntry.profile_id == profile_id, WeightEntry.measured_at.between(first, last)).order_by(WeightEntry.measured_at.desc())
...
def create_weight(profile_id: str, payload: WeightCreate, db: DbSession) -> WeightEntry:
    ...
    entry = WeightEntry(profile_id=profile_id, weight_kg=payload.weight_kg, measured_at=payload.measured_at.astimezone(timezone.utc), source="manual", notes=payload.notes)
    db.add(entry)
    db.commit()
    db.refresh(entry)
    return entry
...
def list_water(profile_id: str, db: DbSession, day: date | None = None) -> list[WaterEntry]:
    ...
    statement = select(WaterEntry).where(WaterEntry.profile_id == profile_id, WaterEntry.consumed_at.between(first, last)).order_by(WaterEntry.consumed_at.desc())
    return list(db.scalars(statement).all())
...
def create_water(profile_id: str, payload: WaterCreate, db: DbSession) -> WaterEntry:
    ...
    entry = WaterEntry(profile_id=profile_id, amount_ml=payload.amount_ml, consumed_at=payload.consumed_at.astimezone(timezone.utc), source="manual", notes=payload.notes)
    db.add(entry)
    db.commit()
    db.refresh(entry)
    return entry
...
def daily_summary(profile_id: str, db: DbSession, day: date = Query(default_factory=date.today)) -> DailySummaryV030:
    ...
    diary_entries = list(db.scalars(select(DiaryEntry).where(DiaryEntry.profile_id == profile_id, DiaryEntry.consumed_at.between(first, last))).all())
    exercise_entries = list(db.scalars(select(ExerciseEntry).where(ExerciseEntry.profile_id == profile_id, ExerciseEntry.completed_at.between(first, last))).all())
    water_entries = list(db.scalars(select(WaterEntry).where(WaterEntry.profile_id == profile_id, WaterEntry.consumed_at.between(first, last))).all())
    ...
    credited, remaining = calorie_budget(daily_calorie_target=profile.daily_calorie_target, consumed_food_calories=consumed, completed_exercise_calories=exercise_calories, exercise_credit_mode=ExerciseCreditMode(profile.exercise_credit_mode), exercise_credit_percentage=profile.exercise_credit_percentage)
    return DailySummaryV030(profile_id=profile_id, date=day, calorie_target=profile.daily_calorie_target, consumed_calories=round(consumed), completed_exercise_calories=round(exercise_calories), credited_exercise_calories=credited, remaining_calories=remaining, exercise_minutes=exercise_minutes, hydration_ml=sum(entry.amount_ml for entry in water_entries), hydration_target_ml=profile.hydration_target_ml, protein_g=round(sum(entry.protein_g or 0 for entry in diary_entries), 1), carbohydrates_g=round(sum(entry.carbohydrates_g or 0 for entry in diary_entries), 1), fat_g=round(sum(entry.fat_g or 0 for entry in diary_entries), 1), sugar_g=round(sum(entry.sugar_g or 0 for entry in diary_entries), 1), entry_count=len(diary_entries))
...
def progress_summary(profile_id: str, db: DbSession, days: int = Query(default=90, ge=7, le=730)) -> ProgressSummary:
    ...
    exercises = list(db.scalars(select(ExerciseEntry).where(ExerciseEntry.profile_id == profile_id, ExerciseEntry.completed_at.between(first, last))).all())
    weights = list(db.scalars(select(WeightEntry).where(WeightEntry.profile_id == profile_id, WeightEntry.measured_at.between(first, last)).order_by(WeightEntry.measured_at.desc())).all())
    week_first, _ = local_day_bounds(local_today - timedelta(days=6), profile.timezone)
    week_exercises = list(db.scalars(select(ExerciseEntry).where(ExerciseEntry.profile_id == profile_id, ExerciseEntry.completed_at.between(week_first, last))).all())
    today_first, today_last = local_day_bounds(local_today, profile.timezone)
    water_today = list(db.scalars(select(WaterEntry).where(WaterEntry.profile_id == profile_id, WaterEntry.consumed_at.between(today_first, today_last))).all())
    latest = weights[0] if weights else None
    change = round(latest.weight_kg - profile.starting_weight_kg, 2) if latest is not None and profile.starting_weight_kg is not None else None
    cutoff = datetime.now(timezone.utc) - timedelta(days=30)
    older = db.scalar(select(WeightEntry).where(WeightEntry.profile_id == profile_id, WeightEntry.measured_at <= cutoff).order_by(WeightEntry.measured_at.desc()).limit(1))
    change_30 = round(latest.weight_kg - older.weight_kg, 2) if latest is not None and older is not None else None
    return ProgressSummary(profile_id=profile_id, period_start=period_start, period_end=local_today, exercise_minutes=sum(entry.duration_minutes for entry in exercises), exercise_minutes_last_7_days=sum(entry.duration_minutes for entry in week_exercises), exercise_minutes_target=profile.weekly_exercise_minutes_target, exercise_calories=round(sum(entry.calories_burned for entry in exercises)), hydration_ml_today=sum(entry.amount_ml for entry in water_today), hydration_target_ml=profile.hydration_target_ml, latest_weight_kg=latest.weight_kg if latest else None, latest_weight_at=latest.measured_at if latest else None, starting_weight_kg=profile.starting_weight_kg, goal_weight_kg=profile.goal_weight_kg, change_from_start_kg=change, change_last_30_days_kg=change_30, weight_entries=[WeightOutput.model_validate(entry) for entry in weights])
