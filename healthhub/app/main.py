...
def food_scale(food: Food) -> float:
    if food.nutrition_basis == "per_100g" and food.serving_grams:
        return food.serving_grams / 100
    if food.nutrition_basis == "per_100ml" and food.canonical_quantity and food.serving_unit.lower() in {"ml", "millilitre", "milliliter"}:
        return food.canonical_quantity / 100
    return 1.0
...
def update_profile(profile_id: str, payload: ProfileUpdate, db: DbSession) -> Profile:
    ...
    if mode == "none": percentage = 0
    elif mode == "full": percentage = 100
    values["exercise_credit_percentage"] = percentage
    for field, value in values.items(): setattr(profile, field, value)
    if nutrition_fields is not None: profile.nutrition_display_fields = nutrition_fields
    db.commit(); db.refresh(profile)
    return profile
...
def archive_profile(profile_id: str, db: DbSession) -> Profile:
    profile = get_profile_or_404(db, profile_id); profile.archived = True; db.commit(); db.refresh(profile); return profile


@app.get("/api/v1/active-profile", response_model=ActiveProfileResponse | None)
def get_active_profile() -> ActiveProfileResponse | None:
    if not ACTIVE_PROFILE_FILE.exists(): return None
    try: return ActiveProfileResponse(profile_id=json.loads(ACTIVE_PROFILE_FILE.read_text(encoding="utf-8"))["profile_id"])
...
def set_active_profile(payload: ActiveProfileSelection, db: DbSession) -> ActiveProfileResponse:
    ...
    if profile.archived: raise HTTPException(status_code=409, detail="Archived profiles cannot be active")
    ACTIVE_PROFILE_FILE.parent.mkdir(parents=True, exist_ok=True); ACTIVE_PROFILE_FILE.write_text(json.dumps({"profile_id": profile.id}), encoding="utf-8")
...
def calculate_budget(payload: CalorieBudgetInput) -> CalorieBudgetOutput:
    credited, remaining = calorie_budget(**payload.model_dump()); return CalorieBudgetOutput(credited_exercise_calories=credited, remaining_calories=remaining)
...
def list_foods(db: DbSession, q: str | None = Query(default=None, min_length=1, max_length=180), favourite: bool | None = None, profile_id: str | None = None, include_archived: bool = False, limit: int = Query(default=50, ge=1, le=200)) -> list[Food]:
    ...
    if not include_archived: statement = statement.where(Food.archived.is_(False))
    if favourite is not None: statement = statement.where(Food.favourite.is_(favourite))
    if q:
    ...
    else: statement = statement.order_by(Food.favourite.desc(), Food.name)
...
def create_food(payload: FoodCreate, db: DbSession) -> Food:
    values = payload.model_dump(mode="json"); barcodes = values.pop("barcodes", [])
    food = Food(**values); db.add(food); db.flush()
    for barcode in barcodes:
        if not valid_barcode(barcode): raise HTTPException(status_code=422, detail=f"Invalid barcode {barcode}")
        db.add(FoodIdentifier(food_id=food.id, identifier_type=barcode_type(barcode), value=barcode))
    db.commit(); db.refresh(food); return food
...
def create_composite_food(payload: CompositeFoodCreate, db: DbSession) -> Food:
    ...
    if any(food.archived for food in components): raise HTTPException(status_code=409, detail="Archived foods cannot be components")
    ...
    for item in payload.components: db.add(FoodComponent(composite_food_id=food.id, component_food_id=item.food_id, quantity=item.quantity, unit=item.unit))
    db.commit(); db.refresh(food); return food
...
def update_food_preference(profile_id: str, food_id: str, payload: FoodPreferenceUpdate, db: DbSession) -> dict[str, object]:
    get_profile_or_404(db, profile_id); get_food_or_404(db, food_id)
    ...
    db.add(preference)
    for field, value in payload.model_dump(exclude_unset=True).items(): setattr(preference, field, value)
    db.commit(); return {"profile_id": profile_id, "food_id": food_id, "favourite": preference.favourite, "default_quantity": preference.default_quantity}
...
def recent_foods(profile_id: str, db: DbSession, limit: int = Query(default=12, ge=1, le=50)) -> list[Food]:
    get_profile_or_404(db, profile_id); return list(db.scalars(select(Food).join(FoodPreference).where(FoodPreference.profile_id == profile_id, FoodPreference.last_used_at.is_not(None)).order_by(FoodPreference.last_used_at.desc()).limit(limit)).all())
...
def preview_food_import(payload: ImportPreviewRequest, db: DbSession) -> ImportPreviewOutput:
    headers, mappings, rows = parse_tsv(payload.tsv, payload.mappings); return preview_rows(headers, mappings, rows, db)
...
async def preview_food_file(db: DbSession, file: Annotated[UploadFile, File(...)], sheet: Annotated[str | None, Form()] = None) -> ImportPreviewOutput:
    ...
    if len(data) > MAX_IMPORT_BYTES: raise HTTPException(status_code=413, detail="Import files must be 5 MB or smaller")
    try:
        ...
        else: raise HTTPException(status_code=415, detail="Upload a .csv or .xlsx file")
    except ValueError as exc: raise HTTPException(status_code=422, detail=str(exc)) from exc
    output = preview_rows(parsed.headers, parsed.mappings, parsed.rows, db)
    return output.model_copy(update={"rows": [{**row, "_source": parsed.source_type, "_source_name": parsed.source_name, "_sheet": parsed.selected_sheet, "_sheet_names": parsed.sheet_names} for row in output.rows]})


@app.post("/api/v1/foods/import/commit")
def commit_food_import(payload: ImportCommitRequest, db: DbSession) -> dict[str, object]:
    batch = ImportBatch(source=payload.source, source_name=payload.source_name); db.add(batch); db.flush()
    ...
    for row_number, row in enumerate(payload.rows, start=2):
        if not row.get("_valid", True):
            batch.rejected_count += 1; errors = row.get("_errors", []); db.add(ImportItem(batch_id=batch.id, row_number=row_number, action="rejected", message="; ".join(str(item) for item in errors) if isinstance(errors, list) else "Invalid row")); continue
        name = str(row.get("name", "")).strip(); brand = str(row.get("brand") or "").strip() or None; serving_size = float(str(row["serving_size"])) if row.get("serving_size") not in (None, "") else None; unit = str(row.get("serving_unit") or "serving")
        match = next((food for food in foods if food_key(food.name, food.brand, food.serving_grams, food.serving_unit) == food_key(name, brand, serving_size, unit)), None)
        if match and payload.duplicate_action == "skip": batch.skipped_count += 1; db.add(ImportItem(batch_id=batch.id, row_number=row_number, food_id=match.id, action="skipped", message="Potential duplicate")); continue
        values: dict[str, object] = {"name": name, "brand": brand, "category": str(row.get("category") or "") or None, "serving_name": f"{serving_size:g} {unit}" if serving_size else f"1 {unit}", "serving_unit": unit, "serving_grams": serving_size, "nutrition_basis": str(row.get("nutrition_basis") or "per_serving"), "canonical_quantity": row.get("canonical_quantity"), "canonical_unit": str(row.get("canonical_unit") or "") or None, "calories": float(str(row.get("calories") or 0)), "source": payload.source, "source_provider": payload.source, "data_quality": "imported", "verification_status": "unverified", "notes": str(row.get("notes") or "") or None}
        aliases = {"carbs_g": "carbohydrates_g"}
        for field in ("protein_g", "carbs_g", "fat_g", "saturated_fat_g", "sugar_g", "fibre_g", "sodium_mg", "calcium_mg", "iron_mg", "potassium_mg", "alcohol_g", "caffeine_mg"): values[aliases.get(field, field)] = row.get(field)
        if match and payload.duplicate_action == "update":
            if quality_rank(match) > 20: batch.skipped_count += 1; db.add(ImportItem(batch_id=batch.id, row_number=row_number, food_id=match.id, action="skipped", message="Existing higher-quality nutrition protected")); continue
            for field, value in values.items(): setattr(match, field, value)
            batch.updated_count += 1; db.add(ImportItem(batch_id=batch.id, row_number=row_number, food_id=match.id, action="updated"))
        else:
            food = Food(**values); db.add(food); db.flush(); foods.append(food); batch.created_count += 1; db.add(ImportItem(batch_id=batch.id, row_number=row_number, food_id=food.id, action="created"))
    db.commit(); return {"batch_id": batch.id, "created": batch.created_count, "updated": batch.updated_count, "skipped": batch.skipped_count, "rejected": batch.rejected_count}
...
async def lookup_barcode(barcode: str, db: DbSession) -> dict:
    ...
    if not valid_barcode(barcode): raise HTTPException(status_code=422, detail="Barcode checksum or length is invalid")
    local = local_barcode_food(db, barcode)
    if local: return {"status": "local", "food": FoodOutput.model_validate(local), "candidate": None}
    provider = product_provider()
    if provider:
        result = await provider.lookup_barcode(barcode)
        if result: return {"status": "external", "food": None, "candidate": product_candidate(result)}
...
def save_product(payload: ProductSaveRequest, db: DbSession) -> Food:
    ...
    values = payload.model_dump(exclude={"reviewed", "provider", "provider_id", "barcode", "confidence", "completeness"})
    calories = values.pop("calories") or 0
    serving_size = values.pop("serving_size")
    serving_unit = values.pop("serving_unit") or ("g" if payload.nutrition_basis == "per_100g" else "mL" if payload.nutrition_basis == "per_100ml" else "serving")
    food = Food(**values, calories=calories, serving_name=f"{serving_size:g} {serving_unit}" if serving_size else "1 serve", serving_unit=serving_unit, serving_grams=serving_size if serving_unit.lower() == "g" else None, canonical_quantity=100 if payload.nutrition_basis in {"per_100g", "per_100ml"} else None, canonical_unit="g" if payload.nutrition_basis == "per_100g" else "mL" if payload.nutrition_basis == "per_100ml" else None, source="external_product_database", source_provider=payload.provider, source_identifier=payload.provider_id, data_quality="external", verification_status="reviewed", verified_at=utc_now())
    db.add(food); db.flush()
    if payload.barcode and valid_barcode(payload.barcode): db.add(FoodIdentifier(food_id=food.id, identifier_type=barcode_type(payload.barcode), value=payload.barcode))
    db.commit(); db.refresh(food); return food


@app.get("/api/v1/foods/{food_id}", response_model=FoodOutput)
def get_food(food_id: str, db: DbSession) -> Food: return get_food_or_404(db, food_id)
...
def update_food(food_id: str, payload: FoodUpdate, db: DbSession) -> Food:
    ...
    for field, value in payload.model_dump(exclude_unset=True, mode="json").items(): setattr(food, field, value)
    db.commit(); db.refresh(food); return food


@app.post("/api/v1/foods/{food_id}/archive", response_model=FoodOutput)
def archive_food(food_id: str, db: DbSession) -> Food:
    food = get_food_or_404(db, food_id); food.archived = True; db.commit(); db.refresh(food); return food
...
def list_diary_entries(profile_id: str, db: DbSession, day: date = Query(default_factory=date.today)) -> list[DiaryEntry]:
    profile = get_profile_or_404(db, profile_id); start, end = local_day_bounds(day, profile.timezone); return list(db.scalars(select(DiaryEntry).where(DiaryEntry.profile_id == profile_id, DiaryEntry.consumed_at.between(start, end)).order_by(DiaryEntry.consumed_at)).all())
...
def create_diary_entry(profile_id: str, payload: DiaryEntryCreate, db: DbSession) -> DiaryEntry:
    profile = get_profile_or_404(db, profile_id)
    if profile.archived: raise HTTPException(status_code=409, detail="Cannot add diary entries to an archived profile")
    food = get_food_or_404(db, payload.food_id)
    if food.archived: raise HTTPException(status_code=409, detail="Archived foods cannot be added to the diary")
    preference = db.get(FoodPreference, {"profile_id": profile_id, "food_id": food.id}); servings = payload.servings; basis_scale = food_scale(food); total_scale = servings * basis_scale
    entry = DiaryEntry(profile_id=profile.id, food_id=food.id, meal_period=payload.meal_period.value, consumed_at=payload.consumed_at.astimezone(timezone.utc), servings=servings, food_name=food.name, serving_name=food.serving_name, calories=round(food.calories * total_scale, 2), protein_g=scale_optional(food.protein_g, total_scale), carbohydrates_g=scale_optional(food.carbohydrates_g, total_scale), fat_g=scale_optional(food.fat_g, total_scale), sugar_g=scale_optional(food.sugar_g, total_scale), saturated_fat_g=scale_optional(food.saturated_fat_g, total_scale), fibre_g=scale_optional(food.fibre_g, total_scale), sodium_mg=scale_optional(food.sodium_mg, total_scale), calcium_mg=scale_optional(food.calcium_mg, total_scale), iron_mg=scale_optional(food.iron_mg, total_scale), potassium_mg=scale_optional(food.potassium_mg, total_scale), cholesterol_mg=scale_optional(food.cholesterol_mg, total_scale), alcohol_g=scale_optional(food.alcohol_g, total_scale), caffeine_mg=scale_optional(food.caffeine_mg, total_scale), source=food.source)
    if preference is None: preference = FoodPreference(profile_id=profile_id, food_id=food.id, use_count=1, last_used_at=utc_now()); db.add(preference)
    else: preference.use_count += 1; preference.last_used_at = utc_now()
...
def delete_diary_entry(profile_id: str, entry_id: str, db: DbSession) -> Response:
    ...
    if not entry: raise HTTPException(status_code=404, detail="Diary entry not found")
...
def daily_summary(profile_id: str, db: DbSession, day: date = Query(default_factory=date.today)) -> DailySummaryOutput:
    profile = get_profile_or_404(db, profile_id); entries = list_diary_entries(profile_id=profile_id, db=db, day=day); consumed = sum(entry.calories for entry in entries); credited, remaining = calorie_budget(daily_calorie_target=profile.daily_calorie_target, consumed_food_calories=consumed, completed_exercise_calories=0, exercise_credit_mode=profile.exercise_credit_mode, exercise_credit_percentage=profile.exercise_credit_percentage)  # type: ignore[arg-type]
...
async def quick_add_search(db: DbSession, q: str = Query(min_length=2, max_length=180), limit: int = Query(default=12, ge=1, le=30), profile_id: str | None = None) -> list[QuickAddResult]:
    ...
    if options.get("enabled", True) and len(results) < limit:
        client = FoodHubClient(options.get("base_url", "http://dinnerhub:8099")); recipes = await client.search_recipes(q, limit=limit - len(results))
...
async def add_foodhub_recipe(profile_id: str, recipe_id: str, db: DbSession, meal_period: str = "dinner", servings: float = 1.0) -> DiaryEntry:
    profile = get_profile_or_404(db, profile_id); options = load_options().get("foodhub", {}); client = FoodHubClient(options.get("base_url", "http://dinnerhub:8099")); recipe = await client.recipe_summary(recipe_id)
    if not recipe or not recipe.nutrition.authoritative or not recipe.nutrition.available: raise HTTPException(status_code=409, detail="FoodHub recipe nutrition is unavailable or not authoritative")
    values = recipe.nutrition.values
    link = db.get(FoodHubRecipeLink, recipe_id); food = db.get(Food, link.food_id) if link else None
    if food is None:
        food = Food(name=recipe.name, kind="composite", serving_name="1 serving", serving_unit="serving", calories=values.get("calories_kcal") or 0, protein_g=values.get("protein_g"), carbohydrates_g=values.get("carbohydrate_g"), fat_g=values.get("fat_g"), saturated_fat_g=values.get("saturated_fat_g"), sugar_g=values.get("sugar_g"), fibre_g=values.get("fibre_g"), sodium_mg=values.get("sodium_mg"), source="foodhub_recipe", source_provider="FoodHub", source_identifier=recipe.id, data_quality="foodhub_derived", verification_status="verified", verified_at=utc_now(), image_url=recipe.image_url)
        db.add(food); db.flush(); link = FoodHubRecipeLink(foodhub_recipe_id=recipe.id, food_id=food.id, recipe_updated_at=recipe.updated_at, nutrition_status="authoritative", last_synced_at=utc_now()); db.add(link)
    elif recipe.updated_at and link and recipe.updated_at != link.recipe_updated_at:
        food.name = recipe.name; food.calories = values.get("calories_kcal") or 0; food.protein_g = values.get("protein_g"); food.carbohydrates_g = values.get("carbohydrate_g"); food.fat_g = values.get("fat_g"); food.saturated_fat_g = values.get("saturated_fat_g"); food.sugar_g = values.get("sugar_g"); food.fibre_g = values.get("fibre_g"); food.sodium_mg = values.get("sodium_mg"); link.recipe_updated_at = recipe.updated_at; link.last_synced_at = utc_now()
...
async def foodhub_status() -> dict:
    options = load_options().get("foodhub", {})
    if not options.get("enabled", True): return {"available": False, "compatible": False, "message": "FoodHub integration is disabled"}
    result = await FoodHubClient(options.get("base_url", "http://dinnerhub:8099")).status(); return {"available": result.available, "compatible": result.compatible, "version": result.version, "message": result.message, "ingredient_mapping_available": False, "ingredient_mapping_message": "Current FoodHub v1 contract exposes authoritative recipe nutrition but not recipe ingredient quantities."}
...
async def upload_nutrition_label(image: Annotated[UploadFile, File(...)]) -> dict:
    if image.content_type not in ALLOWED_CAPTURE_TYPES: raise HTTPException(status_code=415, detail="Upload a JPEG, PNG or WebP image")
    data = await image.read(MAX_CAPTURE_BYTES + 1)
    if not data: raise HTTPException(status_code=422, detail="The uploaded image is empty")
    if len(data) > MAX_CAPTURE_BYTES: raise HTTPException(status_code=413, detail="Nutrition-label images must be 10 MB or smaller")
    try:
        pil = Image.open(io.BytesIO(data)); pil.verify(); pil = Image.open(io.BytesIO(data)).convert("RGB")
    except (UnidentifiedImageError, OSError) as exc: raise HTTPException(status_code=422, detail="Uploaded file is not a valid supported image") from exc
    CAPTURE_DIR.mkdir(parents=True, exist_ok=True); upload_id = str(uuid4()); suffix = ALLOWED_CAPTURE_TYPES[image.content_type]; target = CAPTURE_DIR / f"{upload_id}{suffix}"; target.write_bytes(data)
    try:
        ocr_data = pytesseract.image_to_data(pil, output_type=pytesseract.Output.DICT, config="--psm 6")
        text = "\n".join(" ".join(word for word in ocr_data["text"][index:index + 12] if word.strip()) for index in range(0, len(ocr_data["text"]), 12))
        confidences = [float(value) for value in ocr_data.get("conf", []) if str(value).replace(".", "", 1).isdigit() and float(value) >= 0]
        extraction = parse_nutrition_text(text); average = sum(confidences) / len(confidences) if confidences else 0; overall = "high" if average >= 80 else "needs_review" if average >= 45 else "unknown"
    except (pytesseract.TesseractError, OSError):
        extraction = parse_nutrition_text(""); overall = "unknown"
...
def nutrition_label_image(upload_id: str) -> FileResponse:
    ...
    if not matching: raise HTTPException(status_code=404, detail="Nutrition-label upload not found")
...
def save_reviewed_nutrition_label(payload: NutritionLabelReviewCreate, db: DbSession) -> Food:
    ...
    if not matching: raise HTTPException(status_code=404, detail="Nutrition-label upload not found or already processed")
    ...
    food = Food(name=payload.name, brand=payload.brand, kind=payload.kind.value, serving_name=payload.serving_name, serving_grams=payload.serving_grams, nutrition_basis=payload.nutrition_basis, canonical_quantity=100 if payload.nutrition_basis in {"per_100g", "per_100ml"} else None, canonical_unit="g" if payload.nutrition_basis == "per_100g" else "mL" if payload.nutrition_basis == "per_100ml" else None, energy_kj=payload.energy_kj, calories=payload.calories, protein_g=payload.protein_g, carbohydrates_g=payload.carbohydrates_g, fat_g=payload.fat_g, saturated_fat_g=payload.saturated_fat_g, sugar_g=payload.sugar_g, fibre_g=payload.fibre_g, sodium_mg=payload.sodium_mg, calcium_mg=payload.calcium_mg, iron_mg=payload.iron_mg, potassium_mg=payload.potassium_mg, cholesterol_mg=payload.cholesterol_mg, alcohol_g=payload.alcohol_g, caffeine_mg=payload.caffeine_mg, source="packaging_label", source_provider="local_ocr", data_quality="packaging_confirmed", verification_status="verified", verified_at=utc_now(), ocr_confidence="user_verified", notes="; ".join(warnings) if warnings else None)
    db.add(food); db.flush()
    if payload.barcode:
        barcode = "".join(ch for ch in payload.barcode if ch.isdigit())
        if valid_barcode(barcode):
            existing = local_barcode_food(db, barcode)
            if existing: raise HTTPException(status_code=409, detail={"message": "Barcode already exists", "food_id": existing.id})
            db.add(FoodIdentifier(food_id=food.id, identifier_type=barcode_type(barcode), value=barcode))
    db.commit(); db.refresh(food)
    for item in matching: item.unlink(missing_ok=True)
    return food
...
def frontend(full_path: str):  # type: ignore[no-untyped-def]
    ...
    if full_path and requested.is_file() and requested.resolve().is_relative_to(STATIC_DIR.resolve()): return FileResponse(requested)
