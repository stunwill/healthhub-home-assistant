...
from .models import DiaryEntry, Food, FoodPreference, Profile, utc_now
...
def planned_from_food(
    *,
    ...
    return PlannedEntry(
        ...
        protein_g=scale(food.protein_g, total_scale),
        carbohydrates_g=scale(food.carbohydrates_g, total_scale),
        fat_g=scale(food.fat_g, total_scale),
        sugar_g=scale(food.sugar_g, total_scale),
        source=food.source,
    )
...
def copy_planned_snapshot(entry: PlannedEntry, profile_id: str, planned_for: datetime, meal_period: str | None = None) -> PlannedEntry:
    return PlannedEntry(
        ...
        food_id=entry.food_id,
        meal_period=meal_period or entry.meal_period,
        planned_for=planned_for.astimezone(timezone.utc),
        servings=entry.servings,
        food_name=entry.food_name,
        serving_name=entry.serving_name,
        ...
        sugar_g=entry.sugar_g,
        source=entry.source,
        status=PlannedEntryStatus.PLANNED.value,
    )
...
def create_planned_entry(profile_id: str, payload: PlannedEntryCreate, db: DbSession) -> PlannedEntry:
    ...
    db.commit()
    db.refresh(entry)
    app_ms = round((perf_counter() - started) * 1000, 1)
    entry.performance_ms = app_ms  # type: ignore[attr-defined]
    return entry
...
def consume_planned_entry(profile_id: str, entry_id: str, db: DbSession) -> PlannedEntry:
    ...
    diary = diary_from_planned(entry)
    db.add(diary)
    db.flush()
...
def materialise_rule(db: Session, profile: Profile, food: Food, rule: RecurrenceRule, horizon_days: int = 56) -> int:
    ...
    first = datetime.combine(rule.start_date, time.min, tzinfo=zone).astimezone(timezone.utc)
    last = datetime.combine(end, time.max, tzinfo=zone).astimezone(timezone.utc)
    existing = set(db.scalars(select(PlannedEntry.planned_for).where(PlannedEntry.recurrence_rule_id == rule.id, PlannedEntry.planned_for.between(first, last))).all())
    created = 0
    ...
    while target <= end:
        if recurrence_matches(target, rule):
            ...
            if utc_dt not in existing:
                db.add(planned_from_food(profile=profile, food=food, planned_for=local_dt, meal_period=rule.meal_period, servings=rule.servings, recurrence_rule_id=rule.id))
                existing.add(utc_dt)
                created += 1
...
def create_recurrence_rule(profile_id: str, payload: RecurrenceRuleCreate, db: DbSession) -> RecurrenceRule:
    ...
    food = get_food(db, payload.food_id)
    rule = RecurrenceRule(profile_id=profile.id, food_id=payload.food_id, frequency=payload.frequency.value, meal_period=payload.meal_period.value, servings=payload.servings, start_date=payload.start_date, end_date=payload.end_date, local_time=payload.local_time)
    db.add(rule)
    db.flush()
...
def weekly_plan(profile_id: str, db: DbSession, start: date | None = None) -> WeeklyPlanOutput:
    profile = get_profile(db, profile_id)
    requested = start or profile_today(profile)
    ...
    for item in planned:
        planned_by_day.setdefault(item.planned_for.astimezone(zone).date(), []).append(item)
    for item in consumed:
        consumed_by_day.setdefault(item.consumed_at.astimezone(zone).date(), []).append(item)
    days: list[WeeklyPlanDay] = []
    for offset in range(7):
        current = week_start + timedelta(days=offset)
        p = planned_by_day.get(current, [])
        c = consumed_by_day.get(current, [])
        days.append(WeeklyPlanDay(date=current, planned_calories=round(sum(item.calories for item in p)), planned_count=len(p), consumed_calories=round(sum(item.calories for item in c)), consumed_count=len(c)))
    return WeeklyPlanOutput(profile_id=profile_id, start_date=week_start, end_date=week_end, days=days, planned_calories=sum(day.planned_calories for day in days), consumed_calories=sum(day.consumed_calories for day in days))
...
def day_plan(profile_id: str, db: DbSession, day: date | None = None) -> DailyPlanOutput:
    ...
    return DailyPlanOutput(profile_id=profile_id, date=target, calorie_target=profile.daily_calorie_target, consumed_calories=consumed_calories, planned_calories=planned_calories, remaining_after_planned=profile.daily_calorie_target - consumed_calories - planned_calories, protein_g=round(sum(item.protein_g or 0 for item in [*planned, *consumed]), 1), carbohydrates_g=round(sum(item.carbohydrates_g or 0 for item in [*planned, *consumed]), 1), fat_g=round(sum(item.fat_g or 0 for item in [*planned, *consumed]), 1), sugar_g=round(sum(item.sugar_g or 0 for item in [*planned, *consumed]), 1), planned=planned, consumed=consumed)
...
def copy_entry(profile_id: str, payload: CopyEntryRequest, db: DbSession) -> PlannedEntry:
    profile = get_profile(db, profile_id)
    target_local = datetime.combine(payload.target_date, payload.local_time, tzinfo=ZoneInfo(profile.timezone))
    planned = db.scalar(select(PlannedEntry).where(PlannedEntry.id == payload.entry_id, PlannedEntry.profile_id == profile_id))
    if planned:
        copy = copy_planned_snapshot(planned, profile_id, target_local, payload.meal_period.value if payload.meal_period else None)
    else:
...
def copy_meal(profile_id: str, payload: CopyMealRequest, db: DbSession) -> list[PlannedEntry]:
    ...
    zone = ZoneInfo(profile.timezone)
    planned = list(db.scalars(select(PlannedEntry).where(PlannedEntry.profile_id == profile_id, PlannedEntry.planned_for.between(first, last), PlannedEntry.meal_period == payload.meal_period.value)).all())
    consumed = list(db.scalars(select(DiaryEntry).where(DiaryEntry.profile_id == profile_id, DiaryEntry.consumed_at.between(first, last), DiaryEntry.meal_period == payload.meal_period.value)).all())
    copies: list[PlannedEntry] = []
    for item in planned:
        local = item.planned_for.astimezone(zone)
        copies.append(copy_planned_snapshot(item, profile_id, datetime.combine(payload.target_date, local.timetz().replace(tzinfo=None), tzinfo=zone)))
    for item in consumed:
        local = item.consumed_at.astimezone(zone)
        copies.append(copy_diary_snapshot(item, profile_id, datetime.combine(payload.target_date, local.timetz().replace(tzinfo=None), tzinfo=zone)))
    db.add_all(copies)
    db.commit()
    for item in copies:
        db.refresh(item)
...
def copy_day(profile_id: str, payload: CopyDayRequest, db: DbSession) -> list[PlannedEntry]:
    ...
    first, last = day_bounds(payload.source_date, profile.timezone)
    zone = ZoneInfo(profile.timezone)
    planned = list(db.scalars(select(PlannedEntry).where(PlannedEntry.profile_id == profile_id, PlannedEntry.planned_for.between(first, last))).all())
    consumed = list(db.scalars(select(DiaryEntry).where(DiaryEntry.profile_id == profile_id, DiaryEntry.consumed_at.between(first, last))).all())
    copies: list[PlannedEntry] = []
    for item in planned:
        local = item.planned_for.astimezone(zone)
...
def list_saved_meals(profile_id: str, db: DbSession, include_archived: bool = False) -> list[SavedMeal]:
    get_profile(db, profile_id)
    statement = select(SavedMeal).where(SavedMeal.profile_id == profile_id)
    if not include_archived:
        statement = statement.where(SavedMeal.archived.is_(False))
...
def create_saved_meal(profile_id: str, payload: SavedMealCreate, db: DbSession) -> SavedMeal:
    get_profile(db, profile_id)
    meal = SavedMeal(profile_id=profile_id, name=payload.name.strip(), default_meal_period=payload.default_meal_period.value)
    db.add(meal)
    db.flush()
    for position, item in enumerate(payload.items):
        get_food(db, item.food_id)
        db.add(SavedMealItem(saved_meal_id=meal.id, food_id=item.food_id, servings=item.servings, position=position))
    db.commit()
    db.refresh(meal)
    return meal
...
def update_saved_meal(profile_id: str, meal_id: str, payload: SavedMealUpdate, db: DbSession) -> SavedMeal:
    ...
    if payload.default_meal_period is not None:
        meal.default_meal_period = payload.default_meal_period.value
...
def plan_saved_meal(profile_id: str, meal_id: str, payload: PlannedEntryCreate, db: DbSession) -> list[PlannedEntry]:
    profile = get_profile(db, profile_id)
    meal = db.scalar(select(SavedMeal).where(SavedMeal.id == meal_id, SavedMeal.profile_id == profile_id, SavedMeal.archived.is_(False)))
    if not meal:
        raise HTTPException(status_code=404, detail="Saved meal not found")
    entries: list[PlannedEntry] = []
    for item in sorted(meal.items, key=lambda value: value.position):
        food = get_food(db, item.food_id)
        entry = planned_from_food(profile=profile, food=food, planned_for=payload.planned_for, meal_period=payload.meal_period.value, servings=item.servings * payload.servings)
