...
def food_scale(food: Food) -> float:
    ...
    if (
        food.nutrition_basis == "per_100g"
        and quantity
        and unit in {"g", "gram", "grams"}
    ):
        return quantity / canonical
    if (
        food.nutrition_basis == "per_100ml"
        and quantity
        and unit
        in {"ml", "millilitre", "millilitres", "milliliter", "milliliters"}
    ):
        return quantity / canonical
    return 1.0
...
def update_usage(db: Session, profile_id: str, food_id: str) -> None:
    preference = db.get(
        FoodPreference,
        {"profile_id": profile_id, "food_id": food_id},
    )
...
def create_planned_entry(
    profile_id: str,
    payload: PlannedEntryCreate,
    db: DbSession,
) -> PlannedEntry:
    ...
    if profile.archived:
        raise HTTPException(
            status_code=409,
            detail="Cannot plan entries for an archived profile",
        )
    ...
    db.refresh(entry)
    logger.info(
        "performance operation=add_to_plan duration_ms=%.1f",
...
@router.patch(
    "/profiles/{profile_id}/planned/{entry_id}",
    response_model=PlannedEntryOutput,
)
def update_planned_entry(
    profile_id: str,
    entry_id: str,
    payload: PlannedEntryUpdate,
    db: DbSession,
) -> PlannedEntry:
    entry = db.scalar(
        select(PlannedEntry).where(
            PlannedEntry.id == entry_id,
            PlannedEntry.profile_id == profile_id,
        )
    )
    if not entry:
    ...
    if entry.status != PlannedEntryStatus.PLANNED.value:
        raise HTTPException(
            status_code=409,
            detail="Only planned entries can be edited",
        )
    ...
    meal_period = values.get("meal_period")
    if meal_period is not None:
        entry.meal_period = meal_period.value
    planned_for = values.get("planned_for")
    if planned_for is not None:
        entry.planned_for = planned_for.astimezone(timezone.utc)
...
@router.post(
    "/profiles/{profile_id}/planned/{entry_id}/consume",
    response_model=PlannedEntryOutput,
)
def consume_planned_entry(
    profile_id: str,
    entry_id: str,
    db: DbSession,
) -> PlannedEntry:
    ...
    if entry.status != PlannedEntryStatus.PLANNED.value:
        raise HTTPException(
            status_code=409,
            detail="Only planned entries can be marked consumed",
        )
...
@router.post(
    "/profiles/{profile_id}/planned/{entry_id}/skip",
    response_model=PlannedEntryOutput,
)
def skip_planned_entry(
    profile_id: str,
    entry_id: str,
    db: DbSession,
) -> PlannedEntry:
    ...
    if entry.status != PlannedEntryStatus.PLANNED.value:
        raise HTTPException(
            status_code=409,
            detail="Only planned entries can be skipped",
        )
...
def delete_planned_entry(
    ...
    if entry.status == PlannedEntryStatus.CONSUMED.value:
        raise HTTPException(
            status_code=409,
            detail="Consumed planned entries cannot be deleted",
        )
    db.delete(entry)
    db.commit()
...
def recurrence_matches(target: date, rule: RecurrenceRule) -> bool:
    if target < rule.start_date:
        return False
    if rule.end_date is not None and target > rule.end_date:
        return False
...
def materialise_rule(
    ...
    end = min(rule.end_date, horizon_end) if rule.end_date else horizon_end
    zone = ZoneInfo(profile.timezone)
    hour, minute = (int(part) for part in rule.local_time.split(":"))
    first = datetime.combine(rule.start_date, time.min, tzinfo=zone).astimezone(
        timezone.utc
    )
    ...
    existing = {
        ...
        for value in db.scalars(
            select(PlannedEntry.planned_for).where(
...
@router.get(
    ...
    response_model=list[RecurrenceRuleOutput],
)
def list_recurrence_rules(
    ...
    statement = select(RecurrenceRule).where(
        RecurrenceRule.profile_id == profile_id
    )
...
def create_recurrence_rule(
    ...
    if profile.archived:
        raise HTTPException(
            status_code=409,
            detail="Cannot create recurrence for an archived profile",
        )
    food = get_food(db, payload.food_id)
    rule = RecurrenceRule(
        profile_id=profile.id,
    ...
    db.add(rule)
    db.flush()
    created = materialise_rule(db, profile, food, rule)
    update_usage(db, profile_id, food.id)
    db.commit()
    db.refresh(rule)
    logger.info(
        "performance operation=create_recurrence duration_ms=%.1f materialised=%d",
        (perf_counter() - started) * 1000,
        created,
    )
    return rule
...
@router.post(
    "/profiles/{profile_id}/recurrence/{rule_id}/archive",
    response_model=RecurrenceRuleOutput,
)
def archive_recurrence_rule(
    profile_id: str,
    rule_id: str,
    db: DbSession,
) -> RecurrenceRule:
...
def weekly_plan(
    ...
    for offset in range(7):
        ...
        planned_items = planned_by_day.get(current, [])
        consumed_items = consumed_by_day.get(current, [])
        days.append(
            WeeklyPlanDay(
                date=current,
                planned_calories=round(
                    sum(item.calories for item in planned_items)
                ),
                planned_count=len(planned_items),
                consumed_calories=round(
                    sum(item.calories for item in consumed_items)
                ),
                consumed_count=len(consumed_items),
            )
        )

    output = WeeklyPlanOutput(
        profile_id=profile_id,
        start_date=week_start,
        end_date=week_end,
        days=days,
        planned_calories=sum(day.planned_calories for day in days),
        consumed_calories=sum(day.consumed_calories for day in days),
    )
    logger.info(
        "performance operation=weekly_summary duration_ms=%.1f",
        (perf_counter() - started) * 1000,
    )
    return output
...
@router.post(
    "/profiles/{profile_id}/copy-entry",
    response_model=PlannedEntryOutput,
    status_code=status.HTTP_201_CREATED,
)
def copy_entry(
...
@router.post(
    "/profiles/{profile_id}/copy-meal",
    response_model=list[PlannedEntryOutput],
    status_code=status.HTTP_201_CREATED,
)
def copy_meal(
    profile_id: str,
    payload: CopyMealRequest,
    db: DbSession,
) -> list[PlannedEntry]:
    profile = get_profile(db, profile_id)
    ...
    planned_entries: list[PlannedEntry] = list(
        db.scalars(
            select(PlannedEntry).where(
                PlannedEntry.profile_id == profile_id,
                PlannedEntry.planned_for.between(first, last),
                PlannedEntry.meal_period == payload.meal_period.value,
                PlannedEntry.status == PlannedEntryStatus.PLANNED.value,
            )
        ).all()
    )
    consumed_entries: list[DiaryEntry] = list(
        db.scalars(
            select(DiaryEntry).where(
                DiaryEntry.profile_id == profile_id,
                DiaryEntry.consumed_at.between(first, last),
                DiaryEntry.meal_period == payload.meal_period.value,
            )
        ).all()
    )
...
@router.get(
    "/profiles/{profile_id}/saved-meals",
    response_model=list[SavedMealOutput],
)
...
@router.post(
    "/profiles/{profile_id}/saved-meals",
    response_model=SavedMealOutput,
    status_code=status.HTTP_201_CREATED,
)
def create_saved_meal(
    ...
    meal = SavedMeal(
        ...
        default_meal_period=payload.default_meal_period.value,
    )
    db.add(meal)
    db.flush()
...
@router.patch(
    "/profiles/{profile_id}/saved-meals/{meal_id}",
    response_model=SavedMealOutput,
)
def update_saved_meal(
    ...
    meal = db.scalar(
        select(SavedMeal).where(
            ...
            SavedMeal.profile_id == profile_id,
        )
    )
    if not meal:
...
@router.post(
    "/profiles/{profile_id}/saved-meals/{meal_id}/archive",
    response_model=SavedMealOutput,
)
def archive_saved_meal(
    profile_id: str,
    meal_id: str,
    db: DbSession,
) -> SavedMeal:
...
@router.post(
    "/profiles/{profile_id}/saved-meals/{meal_id}/plan",
    response_model=list[PlannedEntryOutput],
    status_code=status.HTTP_201_CREATED,
)
def plan_saved_meal(
    ...
    for item in sorted(meal.items, key=lambda value: value.position):
        ...
        entry = planned_from_food(
            ...
            planned_for=payload.planned_for,
            meal_period=payload.meal_period.value,
            servings=item.servings * payload.servings,
        )
