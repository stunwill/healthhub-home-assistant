...
def test_plan_then_consume_creates_diary_snapshot() -> None:
    ...
    consumed = client.post(f"/api/v1/profiles/{profile_id}/planned/{planned.json()['id']}/consume")
...
def test_skip_planned_entry_does_not_create_diary_entry() -> None:
    ...
    skipped = client.post(f"/api/v1/profiles/{profile_id}/planned/{planned.json()['id']}/skip")
...
def test_weekday_recurrence_materialises_only_weekdays() -> None:
    ...
    planned = client.get(f"/api/v1/profiles/{profile_id}/planned?start=2026-08-10&days=7&include_completed=false")
...
def test_weekly_plan_summarises_planned_and_consumed() -> None:
    ...
    client.post(
        f"/api/v1/profiles/{profile_id}/planned",
        json={"food_id": food_id, "meal_period": "dinner", "planned_for": "2026-08-12T18:00:00+10:00", "servings": 1},
    )
    client.post(
        f"/api/v1/profiles/{profile_id}/diary",
        json={"food_id": food_id, "meal_period": "lunch", "consumed_at": "2026-08-11T12:00:00+10:00", "servings": 1},
    )
...
def test_planned_datetime_requires_timezone() -> None:
    ...
    response = client.post(
        f"/api/v1/profiles/{profile_id}/planned",
        json={"food_id": food_id, "meal_period": "snack", "planned_for": datetime(2026, 8, 15, 10, 0).isoformat(), "servings": 1},
    )
    assert response.status_code == 422


def test_recurrence_end_before_start_rejected() -> None:
    profile_id = create_profile()
    food_id = create_food()
    response = client.post(
        f"/api/v1/profiles/{profile_id}/recurrence",
