...
function addDays(day: string, offset: number) { const value = new Date(`${day}T12:00:00`); value.setDate(value.getDate() + offset); return new Intl.DateTimeFormat('en-CA').format(value) }
function displayDay(day: string) { return new Intl.DateTimeFormat('en-AU', { weekday: 'long', day: 'numeric', month: 'long' }).format(new Date(`${day}T12:00:00`)) }
function legacyMode(fields: NutritionField[]): Profile['nutrition_display_mode'] { if (fields.length === 1 && fields[0] === 'calories') return 'simple'; if (fields.length === 2 && fields.includes('calories') && fields.includes('protein')) return 'balanced'; if (['calories', 'protein', 'carbohydrates', 'fat'].every((field) => fields.includes(field as NutritionField))) return 'detailed'; return 'simple' }
function normaliseMeal(value: MealPeriod): MealPeriod { return value === 'snack' ? 'afternoon_snack' : value }

function NutritionMultiSelect({ value, onChange }: { value: NutritionField[]; onChange: (value: NutritionField[]) => void }) {
  function toggle(field: NutritionField) { if (value.includes(field)) { if (value.length === 1) return; onChange(value.filter((item) => item !== field)) } else onChange([...value, field]) }
...
export default function App() {
  ...
  const [mealPeriod, setMealPeriod] = useState<MealPeriod>('afternoon_snack')
  const [quickAddMode, setQuickAddMode] = useState<'eaten' | 'planned'>('eaten')
  ...
  async function editPlanned(entry: PlannedEntry) { if (!activeProfileId) return; const servings = window.prompt('Servings', String(entry.servings)); if (servings == null) return; const meal = window.prompt('Meal section', normaliseMeal(entry.meal_period)); if (meal == null) return; const response = await fetch(`${API}/profiles/${activeProfileId}/planned/${entry.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ servings: Number(servings), meal_period: meal }) }); if (response.ok) await loadDay(activeProfileId, selectedDay); else setNotice('Could not update planned item') }
  async function editDiary(entry: DiaryEntry) { if (!activeProfileId) return; const servings = window.prompt('Servings', String(entry.servings)); if (servings == null) return; const meal = window.prompt('Meal section', normaliseMeal(entry.meal_period)); if (meal == null) return; const response = await fetch(`${API}/profiles/${activeProfileId}/diary/${entry.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ servings: Number(servings), meal_period: meal }) }); if (response.ok) await loadDay(activeProfileId, selectedDay); else setNotice('Could not update diary item') }
  ...
  const profileForm = <section className="profile-onboarding"><p className="eyebrow">Profile</p><h1>Create your first profile</h1><p>Profiles separate household nutrition records. They are data selectors, not secure accounts.</p><div className="food-form"><label>Display name<input autoFocus value={profileDraft.display_name} onChange={(e) => setProfileDraft({ ...profileDraft, display_name: e.target.value })} /></label><label>Daily calorie target<input inputMode="numeric" value={profileDraft.daily_calorie_target} onChange={(e) => setProfileDraft({ ...profileDraft, daily_calorie_target: e.target.value })} /></label><label>Weekly exercise target (minutes)<input inputMode="numeric" value={profileDraft.weekly_exercise_minutes_target} onChange={(e) => setProfileDraft({ ...profileDraft, weekly_exercise_minutes_target: e.target.value })} /></label><label>Hydration target (mL, optional)<input inputMode="numeric" value={profileDraft.hydration_target_ml} onChange={(e) => setProfileDraft({ ...profileDraft, hydration_target_ml: e.target.value })} /></label><label>Exercise calorie credit<select value={profileDraft.exercise_credit_mode} onChange={(e) => setProfileDraft({ ...profileDraft, exercise_credit_mode: e.target.value as ProfileDraft['exercise_credit_mode'] })}><option value="none">No exercise credit</option><option value="full">Full exercise credit</option><option value="percentage">Percentage exercise credit</option></select></label>{profileDraft.exercise_credit_mode === 'percentage' && <label>Exercise credit percentage<input inputMode="numeric" value={profileDraft.exercise_credit_percentage} onChange={(e) => setProfileDraft({ ...profileDraft, exercise_credit_percentage: e.target.value })} /></label>}<label>Nutrition display<NutritionMultiSelect value={profileDraft.nutrition_display_fields} onChange={(fields) => setProfileDraft({ ...profileDraft, nutrition_display_fields: fields })} /></label></div><button className="quick-add" disabled={savingProfile} onClick={() => void createProfile()}>{savingProfile ? 'Creating…' : 'Create profile'}</button></section>
